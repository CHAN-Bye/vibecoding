...
requirements.txt 파일에 다음의 내용을 포함
...
```bash
fastmcp install claude-desktop analytics-mcp.py --with-requirements requirements.txt
```

```text
analytics-mcp 서버를 사용하여 데이터프레임에서 중복값을 제거한 후, 결측치도 모두 제거해줘.
```
